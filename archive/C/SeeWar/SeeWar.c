...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
        case WM_ACTIVATE:  /* Àêòèâíîå îêíî? */
        {
            g_fActive = !HIWORD(wParam);

            if(player==1)
				SetDlgItemText(hwnd,LABEL,"Ïåðâûé èãðîê, ââåäèòå ïàðîëü");
			else
				SetDlgItemText(hwnd,LABEL,"Âòîðîé èãðîê, ââåäèòå ïàðîëü");
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    if (!CreateGLWindow("Ìîðñêîé áîé", 640, 480, 16, FALSE))
        return 0;
	ClearBoards();
	player=1;
            ...
            else  /* Not Time To Quit, Update Screen */
            {
                ...
                if (g_fKeys[' '])
                {
					...
					if(player==1) 
					{
						...
						do{
							...
							if(strcmp(buffer,password2)!=0)
								MessageBox(0,"Ïàðîëü íå âåðåí","Îøèáêà",MB_OK);
					...
					if(HodP1==HodP2&&HodP1==1)
					{
						...
						{
							MessageBox(0,"Âòîðîé èãðîê ïîáåäèë","Ìîðñêîé áîé",MB_ICONINFORMATION );
							ClearBoards();
							player=1;
							CreateBoards();
							player=2;
						...
						{
							MessageBox(0,"Ïåðâûé èãðîê ïîáåäèë","Ìîðñêîé áîé",MB_ICONINFORMATION );
							ClearBoards();
							player=1;
							CreateBoards();
							player=2;
						...
						if(DeathP2==4*1+3*2+2*3+4*1&&DeathP1==18)
						{
							MessageBox(0,"Íè÷üÿ","Ìîðñêîé áîé",MB_ICONINFORMATION );
							ClearBoards();
							player=1;
							CreateBoards();
							player=2;
