...
void __fastcall TMainForm::US1Click(TObject *Sender)
{
  ...
  if(LoadNewResourceModule(dynamic_cast<TComponent*>(Sender)->Tag) != 0)
  {
    ...
    //Russian1->Checked = Russian1== Sender;
    CurrText()->Name = Name;
    CurrText()->Size = Size;
    SelectionChange(this);
    ...
    SetupRuler();
    RichEdit1->SetFocus();
    // Переменнование PopupMenu
    Undo1->Caption=EditUndo->Caption;   // Перевод заголова
    Undo1->Hint=EditUndo->Hint;         // Перевод подсказки

    Cut1->Caption=EditCut->Caption;     // Перевод заголова
    Cut1->Hint=EditCut->Hint;           // Перевод подсказки

    Copy1->Caption=EditCopy->Caption;   // Перевод заголова
    Copy1->Hint=EditCopy->Hint;         // Перевод подсказки

    Paste1->Caption=EditPaste->Caption; // Перевод заголова
    Paste1->Hint=EditPaste->Hint;       // Перевод подсказки

    Font1->Caption=EditFont->Caption;   // Перевод заголова
    Font1->Hint=EditFont->Hint;         // Перевод подсказки
